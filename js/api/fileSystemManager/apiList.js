...
module.exports = {
    ...
    copyFile(obj) {
        fs.copyFile({
            srcPath: `${obj.path}`,
            destPath: `${qq.env.USER_DATA_PATH}/fileA/fileB/content.txt`,
            success: () => {
                obj.callback && obj.callback(`成功拷贝到文件路径 ${qq.env.USER_DATA_PATH}/fileA/fileB/content.txt`);
            },
            fail: res => {
                if (!obj.callback) return;
                for (var i = 0, errMsglist = Object.keys(errMsgObj); i < errMsglist.length; i++) {
    ...
    rename(obj) {
        fs.rename({
            oldPath: `${obj.path}`,
            newPath: `${qq.env.USER_DATA_PATH}/fileA/fileB/afterRename.txt`,
            success: () => {
                obj.callback &&
                    obj.callback(
                        `成功重命名到文件路径 ${
